...
const OtherExpenseRecord = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios.get(`/getExpensesRecordOthers`);
    ...
    <>
      {data.length === 0 ? (
        ...
        <>
          ...
          {isPopupHealth && (
            <ExpensesPopupForm id={selectedId} comment="Add Other Expenses" action="other" onClose={() => handleIncomePopup(null)} />
          )}
        </>
      )}
    </>
...
export default OtherExpenseRecord;
