...
const FeedsExpenseRecord = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios.get(`/getExpensesRecordFeeds`);
    ...
    <>
      {data.length === 0 ? (
        ...
        <>
          ...
          {isPopupHealth && (
            <ExpensesPopupForm id={selectedId} comment="Add Feeds Expense" action="feeds" onClose={() => handleIncomePopup(null)} />
          )}
        </>
      )}
    </>
...
export default FeedsExpenseRecord;
