...
const EggRecordBody = ({ dataItem, onPopup, check }) => (
  <Grid key={dataItem.id} item xs={12} sm={check == 1 ? 12 : 6}>
    <SubCard
      title={dataItem.id}
      secondary={<SecondaryAction title="Add Egg Record" icon={<AddIcon />} onClickIcon={() => onPopup(dataItem.id)} />}
    >
            ...
            {dataItem.totalEgg > 0 ? (
              <Paper style={{ height: 200, width: '100%', overflowX: 'auto' }}>
                <Table>
                  ...
                  <TableBody>
                    {dataItem.eggProduction.map((dataItem2, index) => (
                      <TableRow key={index}>
                    ...
                    <TableRow>
                      ...
                      <TableCell align="center">{`${dataItem.totalEgg}`}</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </Paper>
...
EggRecordBody.propTypes = {
  dataItem: PropTypes.array,
  onPopup: PropTypes.func,
  check: PropTypes.bool
};

export default EggRecordBody;
